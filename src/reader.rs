...
pub fn read_input(filename: &str) -> Option<()> {
    ...
    let mut resources_per_mode: Vec<Vec<u32>> = vec![vec![Default::default(); n_renewable + n_nonrenewable]; modes.len()];

    skip_lines(&mut lines, 4);
    for mode in 0..modes.len() {
        let line = line_to_numbers(lines.next()?.ok()?);
        durations_per_mode.push(*line.get(line.len() - (n_renewable + n_nonrenewable + 1)).unwrap());
        let resources_mode = resources_per_mode.get_mut(mode).unwrap();
        for i in 0..n_renewable {
            resources_mode[i] = *line.get(line.len() + i - n_nonrenewable - n_renewable).unwrap();
        }
